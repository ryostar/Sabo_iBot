...
async def pause(_, message: Message):
    ...
    if (chat_id not in callsmusic.pytgcalls.active_calls) or (
        ...
        await message.reply_text("❗ Không có gì để dừng!")
    else:
...
async def resume(_, message: Message):
    ...
    if (chat_id not in callsmusic.pytgcalls.active_calls) or (
        ...
        await message.reply_text("❗ Không có nhạc đang tạm dừng!")
    else:
        callsmusic.pytgcalls.resume_stream(chat_id)
        await message.reply_text("⏸ Đã tiếp tục phát nhạc!")


@Client.on_message(command("off") & other_filters)
@errors
@authorized_users_only
async def stop(_, message: Message):
    chat_id = get_chat_id(message.chat)
    if chat_id not in callsmusic.pytgcalls.active_calls:
        await message.reply_text("❗ Không có trên cuộc gọi thoại!")
    else:
        try:
